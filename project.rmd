...
model {
  ...
  for (i in 1:((n-n_test)-1)) {
    y[i] ~ dnorm(mu[i], tau) T(0,60)
    mu[i] = b0 + inprod(b, x[i,])
  }

  for (k in (n-n_test):n) {
    y_test[k] ~ dnorm(mu_test[k], tau) T(0,60)
    mu_test[k] = b0 + inprod(b, x[k,])
  }
}
"
...
samples = coda.samples(model, 
                       variable.names = c("b0","sd","b", "y_test"),
                       n.iter = 1000 * 30, thin = 30)
...
The model was difficult to configure as $b0$, $b1$ and $b2$ had mixing issues observable in the trace plot, which only improved after a burn in and thinning. The level thinning was decided on using the autocorrelation plot which showed a need of thinning every 30th value for $b0$! When it came to other metrics, effective size reflected the high correlation with the $beta$ parameters - around 460 for $B1$ and 308 for $b0$, nearly half of other parameters, but siginifcantly improved after thinning - most parameters around 3900-4300. Convergence was not a problem according to gelman diag diagnostics. 
...
par(mfrow = c(1,2))
hist(colMeans(samples[[1]][,8:57]))
hist(Reisby$hd[201:50])
